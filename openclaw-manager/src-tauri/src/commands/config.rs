...
/// 备份用户配置
#[command]
pub async fn backup_user_config() -> Result<String, String> {
    info!("[配置备份] 开始备份用户配置...");
    ...
    match backup_openclaw_dir(&home)? {
        ...
        None => Ok("OpenClaw 配置目录不存在，已跳过备份".to_string()),
    }
}
...
mod tests {
    ...
    #[test]
    fn backup_copies_files_when_openclaw_dir_exists() {
        let home = make_temp_dir("openclaw_home_exists");
        let openclaw_dir = home.join(".openclaw");
