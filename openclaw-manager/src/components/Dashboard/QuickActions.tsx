import { Play, Square, RotateCcw, Stethoscope, Globe } from 'lucide-react';
...
export function QuickActions({
  ...
  const port = status?.port || 8789;

  const handleOpenWeb = async () => {
    try {
      await open(`http://127.0.0.1:${port}`);
    } catch (e) {
      console.error('无法打开浏览器:', e);
    }
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <button
          ...
          className={clsx(
            ...
            !isRunning
              ? 'bg-dark-600 opacity-50 cursor-not-allowed'
              : 'bg-dark-600 hover:bg-blue-500/20 hover:border-blue-500/50'
          )}
        >
          <div className="w-12 h-12 rounded-full flex items-center justify-center bg-blue-500/20">
            <Globe size={20} className="text-blue-400" />
          </div>
          <span className="text-sm font-medium text-gray-300">打开 Web</span>
        </button>
      </div>
    </div>
  );
