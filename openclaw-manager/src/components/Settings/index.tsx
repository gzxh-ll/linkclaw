...
export function Settings({ onEnvironmentChange }: SettingsProps) {
  ...
  const handleSyncGithub = async () => {
    if (!confirm('警告：此操作将从 服务器上 拉取最新的开发代码，可能包含不稳定的功能。\n\n确定要继续吗？')) {
      return;
    }
    
    setSyncing(true);
    try {
      const result = await invoke<InstallResult>('sync_openclaw_github');
      if (result.success) {
        alert('同步成功！\n' + result.message);
        onEnvironmentChange?.();
      } else {
        ...
        <div className="bg-dark-700 rounded-2xl p-6 border border-dark-500">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <div className="flex-1">
                <p className="text-sm text-white">同步 GitHub (开发版)</p>
                <p className="text-xs text-gray-500">拉取最新代码并重新安装</p>
              </div>
              {syncing && <Loader2 size={18} className="text-blue-400 animate-spin" />}
            </button>
          </div>
        </div>
