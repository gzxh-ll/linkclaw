...
export function Sidebar({ currentPage, onNavigate, serviceStatus }: SidebarProps) {
  ...
  useEffect(() => {
    invoke<string>('check_environment').then((status: any) => {
      if (status && status.os) {
        setPlatform(status.os === 'windows' ? 'Windows' : status.os === 'macos' ? 'macOS' : status.os);
      }
    ...
    <aside className="w-64 bg-dark-800 border-r border-dark-600 flex flex-col">
      ...
      <div className="p-4 border-t border-dark-600 space-y-3">
        ...
        <div className="px-4 py-3 bg-dark-700 rounded-lg">
          ...
          <p className="text-xs text-gray-500">端口: {serviceStatus?.port ?? 8789}</p>
        </div>

        {/* 平台标识 */}
        <div className="px-2 text-center">
          <p className="text-[10px] text-dark-400">
            OpenClaw Manager for {platform || '...'}
          </p>
        </div>
      </div>
    </aside>
